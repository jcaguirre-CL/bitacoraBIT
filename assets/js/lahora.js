			function updateClock() {
    			var now = new Date(), // current date
        			...
        			time = now.getHours() + ':' + now.getMinutes() + ':' + now.getSeconds(); // again, you get the idea
							timeArticle = now.getHours() + ':' + now.getMinutes();
        			...
        			document.getElementById('fecha').innerHTML = [date, time].join(' / ');
							document.getElementById('tiempoArticle1').innerHTML = [date, timeArticle].join(' / ');
							document.getElementById('tiempoArticle2').innerHTML = [date, timeArticle].join(' / ');
